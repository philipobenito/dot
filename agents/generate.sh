...
# Generates agent files for different AI coding tools from a single source
#
# Supported tools:
#   - Claude Code (.claude/agents/)
#   - OpenCode (.config/opencode/agents/)
#
# Run './agents/generate.sh format' to see full source format documentation
...
parse_agent_file() {
    ...
    log_debug "Parsed: model=$MODEL mode=$MODE temp=$TEMPERATURE tools=${TOOLS_ARRAY[*]:-none}"
}
...
clean() {
    log_info "Cleaning generated agent files..."
...
# Generate all agents
generate_all() {
    log_info "Generating agents from source..."
    echo ""
    ...
    local count=0
    for source_file in "$SCRIPT_DIR"/*.md; do
    ...
    echo ""
    log_info "Generated $count agents for each tool"
    echo ""
...
# List source agents with descriptions
list_agents() {
    log_info "Source agents in $SCRIPT_DIR:"
    echo ""
...
Commands:
    generate    Generate agents for all tools (default)
    clean       Remove all generated agent files
...
Examples:
    $(basename "$0")              # Generate all agents
    $(basename "$0") list         # List agents with models
